...
public class Ex04 {
    ...
    void init(){
        ...
        for (int i=1; i<=10; i++){
            ...
            member.setPassword(faker.regexify("[a-zA-Z]{8,16}"));
    ...
    void test1(){
        ...
        System.out.printf("total=%d, pages= %d", total, pages);
    }

    @Test
    void test2(){
        //Pageable pageable = PageRequest.of(1,3, Sort.by(Sort.Order.desc("regDt"), Sort.Order.asc("email"))); // 1-2 페이지
        Pageable pageable = PageRequest.of(1,3, Sort.by(desc("regDt"), asc("email"))); // 1-2 페이지
