...
public class FileUploadService {
    private final FileProperties properties; // 설정 주입
    private final FileInfoRepository fileInfoRepository;
    private final FileInfoService infoService;

    public List<FileInfo> upload(RequestUpload form) {
        String gid = form.getGid();
        gid = StringUtils.hasText(gid) ? gid : UUID.randomUUID().toString(); // gid 없으면 하나 만듬

        String location = form.getLocation(); // 필수는 아닌데 있으면 표기
        MultipartFile[] files = form.getFiles();

        String rootPath = properties.getPath(); // 설정에서 경로 따오기
        ...
        for (MultipartFile file : files) {
            ...
            String fileName = file.getOriginalFilename(); // 이름 넣기
            String extension = fileName.substring(fileName.lastIndexOf(".")); // 확장자 넣기
            ...
            long folder = seq % 10L; // 0 ~ 9, 10개 폴더로 나누어 파일을 저장, 단일 폴더 부하 저감
