...
public class ApiFileController {
    ...
    public JSONData upload(@RequestPart("file") MultipartFile[] files, @Valid RequestUpload form, Errors errors) {
        ...
        form.setFiles(files); // multipartfiles
         ...
         */
        if (form.isSingle()) {
            deleteService.deletes(form.getGid(), form.getLocation()); // 삭제
        }

        List<FileInfo> uploadedFiles = uploadService.upload(form); // 업로드, 요청쪽에서도 처리 가능하게 함?

        // 업로드 완료 하자마자 완료 처리
        if (form.isDone()) {
            doneService.process(form.getGid(), form.getLocation()); // 되면 완료처리
        }
     ...
     * gid, location
     */
    @GetMapping(path={"/list/{gid}", "/list/{gid}/{location}"}) // 주소가 두가지, location은 비필수
