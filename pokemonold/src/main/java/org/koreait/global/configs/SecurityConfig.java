...
public class SecurityConfig {
    ...
    @Bean
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception { // 로그인 양식 처리 주소
        ...
        http.formLogin(c -> {
           c.loginPage("/member/login") // 로그인 양식을 처리할 주소
                   ...
                   .failureHandler(new LoginFailureHandler()) // 실패 시 호출되는거?
         ...
         */
            http.authorizeHttpRequests(c -> {
                c.requestMatchers("/mypage/**").authenticated() // 인증한 회원 "/member/agree").anonymous() // 미인증 회원
        ...
        http.exceptionHandling(c -> {
            c.authenticationEntryPoint(new MemberAuthenticationExceptionHandler())  // 미로그인시 인가 실패
                    .accessDeniedHandler(new MemberAccessDeniedHandler()); // 로그인 이후 인가 실패 기본 403, 여기선 401로 설정(MemberAuthenticationExceptionHandler())
        ...
        http.rememberMe(c->{
            c.rememberMeParameter("autoLogin")
                    ...
                    .authenticationSuccessHandler(new LoginSuccessHandler()); // 자동로그인 유지 기간, 기본값 14일
        });

        /* 자동 로그인 설정 E */

        return http.build(); // 설정 객체 빌드 후 반환
    }
