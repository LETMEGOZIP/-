...
public class Utils {
    ...
    public String tpl(String path) {
        String prefix = isMobile() ? "mobile" : "front"; //주소 받아와서 모바일인지 판별
    ...
    public Map<String, List<String>> getErrorMessages(Errors errors) {
        ResourceBundleMessageSource ms = (ResourceBundleMessageSource) messageSource; // 형변환 후
        ms.setUseCodeAsDefaultMessage(false); // 코드뱉기를 폴스로 변경
        try {
    ...
    public String showImage(Long seq, String url, int width, int height, String mode, String className) {
        ...
        try {
            ...
            } else if (StringUtils.hasText(url)) { // 원격 주소로 썸네일
